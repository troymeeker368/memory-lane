...
export function MhpAllergiesSection({
  memberId,
  initialRows
}: {
  memberId: string;
  ...
  const handleAdd = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      const result = await addMhpAllergyInlineAction(formData);
  ...
  const handleDelete = (allergyId: string) => {
    ...
    startTransition(async () => {
      ...
      const result = await deleteMhpAllergyInlineAction(formData);
      if (!result.ok) {
  ...
  const handleEditSave = () => {
    ...
    startTransition(async () => {
      ...
      const result = await updateMhpAllergyInlineAction(formData);
      if (!result.ok || !result.row) {
