...
import { submitPublicCarePlanSignatureAction } from "@/app/sign/care-plan/[token]/actions";

export function CarePlanPublicSignForm({
  token,
  caregiverNameDefault
}: {
  token: string;
  caregiverNameDefault: string;
}) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const drawingRef = useRef(false);
  const [caregiverTypedName, setCaregiverTypedName] = useState(caregiverNameDefault);
  ...
  function submitSignature() {
    ...
    if (!caregiverTypedName.trim()) {
      setStatus("Typed caregiver name is required.");
      return;
    }
    if (!hasSignature) {
    ...
    startTransition(async () => {
      ...
      formData.set("caregiverTypedName", caregiverTypedName.trim());
      formData.set("signatureImageDataUrl", signatureImageDataUrl);
      formData.set("attested", attested ? "true" : "false");
      const result = await submitPublicCarePlanSignatureAction(formData);
      if (!result.ok) {
  ...
  return (
    <div className="space-y-3">
      <label className="block text-sm">
        <span className="text-xs font-semibold text-muted">Caregiver Typed Name</span>
        <input
      ...
      <label className="flex items-start gap-2 text-sm">
        <input
          ...
          disabled={isPending}
        />
        <span>I attest this is my electronic signature and I approve this care plan.</span>
      </label>
