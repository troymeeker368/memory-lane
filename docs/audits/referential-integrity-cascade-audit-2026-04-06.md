...
- current runtime copy still tells staff not to treat signed orders as operationally ready until sync resolves
Impact:
...
- current worktree extracts shell backfill into `lib/services/member-command-center-repair-supabase.ts` and keeps `scripts/repair-historical-drift.ts` as the explicit repair entry point
Impact:
...
- enrollment packet `completed` still requires `mapping_sync_status = completed` and `completion_follow_up_status = completed` before it is actually ready
...
Evidence:
- `enrollment_packet_requests.member_id` remains required in the canonical workflow contract
- packet preparation/finalization paths still operate on a canonical member-backed packet, not a lead-only packet completion
Assessment:
...
Evidence:
- `supabase/migrations/0006_intake_pof_mhp_supabase.sql`
- `supabase/migrations/0181_physician_order_save_rpc_atomicity.sql` hardens the canonical RPC path with row locks and member mismatch checks, but Postgres still does not enforce same-member lineage at the FK level
Risk:
...
10. The recent explicit MCC repair extraction is operationally safer, but it does not close the remaining schema lineage gaps above.
Evidence:
- current worktree moves shell backfill into `lib/services/member-command-center-repair-supabase.ts`
Risk:
- runtime behavior is clearer, but cross-member drift in unresolved snapshot/sidecar tables is still prevented mainly by service discipline rather than DB enforcement
...
The good news is the obvious child-table breakpoints still look solid. Enrollment packet child rows, intake follow-up rows, POF medications, MAR schedules/administrations, and care-plan diagnoses all still have real database-backed lineage protection. I did not find a new regression that re-opened those bigger referential integrity holes.

The remaining problem is narrower but still important: several snapshot and sidecar tables still prove only the parent record id, not that the parent belongs to the same member. The biggest unresolved example is still `physician_orders.intake_assessment_id`. The April 6 repo state keeps tightening canonical runtime behavior, especially around explicit MCC shell repair, but it still has not moved those remaining same-member lineage guarantees into Postgres.

Production impact:
- low immediate evidence of a new regression on canonical write paths
- medium residual risk if any future side-path, script, or bug bypasses the canonical RPC/service layer and writes cross-member references that still satisfy simple FKs
- workflow truth is safer than before, but staff still cannot treat raw `completed` or `signed` states as operational readiness
