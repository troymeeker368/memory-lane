...
- Enrollment-packet completion consensus and follow-up truth handling are stricter in [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-follow-up.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-follow-up.ts) and [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts): incomplete downstream agreement now drives `completion_follow_up_status = 'action_required'`, and failed follow-up-state persistence throws instead of silently returning a healthier result.
- The main structural gaps from May 10, 2026 remain open: missing cross-member composite lineage constraints at intake and POF boundaries, queue-backed partial lifecycle truth, and downstream sync RPCs that still recreate missing `member_health_profiles` shells.
...
1. Enrollment packet `completed` still does not mean downstream member conversion and follow-up are finished.
   ...
   [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts) now hardens the follow-up truth, but it still operates after the canonical packet row has already been finalized as `completed`.
   Impact:
...
4. Enrollment packet `completed` remains a partial truth without follow-up completion.
   ...
   [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-follow-up.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-follow-up.ts) now improves consensus reporting, but the top-level packet status is still not sufficient by itself.
   Impact:
...
1. `assessment_responses(assessment_id, member_id) -> intake_assessments(id, member_id)` is still missing.
   ...
   [`/D:/Memory Lane App/types/supabase-types.d.ts`](/D:/Memory Lane App/types/supabase-types.d.ts) still exposes only separate `assessment_responses_assessment_id_fkey` and `assessment_responses_member_id_fkey`.
   Risk:
...
3. `member_health_profiles(active_physician_order_id, member_id) -> physician_orders(id, member_id)` is still missing.
   ...
   [`/D:/Memory Lane App/types/supabase-types.d.ts`](/D:/Memory Lane App/types/supabase-types.d.ts) still exposes only `member_health_profiles_active_physician_order_id_fkey`.
   Risk:
...
5. `pof_post_sign_sync_queue(physician_order_id, member_id) -> physician_orders(id, member_id)` is still missing.
   ...
   [`/D:/Memory Lane App/types/supabase-types.d.ts`](/D:/Memory Lane App/types/supabase-types.d.ts) still exposes only separate `pof_post_sign_sync_queue_member_id_fkey` and `pof_post_sign_sync_queue_physician_order_id_fkey`.
   Risk:
...
The current worktree did not introduce a new structural break in the main lead-to-MAR chain. The most meaningful improvement since May 10, 2026 is that enrollment-packet completion truth is stricter: downstream disagreement now pushes the packet into explicit follow-up instead of quietly returning a healthier-looking result.

The main production risk is still that some lifecycle states can look complete before their downstream work is actually complete. A packet can be marked `completed` while member conversion and follow-up are still pending. A POF can be signed while downstream clinical sync is still queued or failed. That is manageable only if every consumer respects the secondary status fields, and that is still easy to get wrong.

The deeper architecture issue is unchanged: several tables still point at valid parent rows without the database guaranteeing that those parent rows belong to the same member. The next safe move is still to add the missing composite lineage constraints first, then re-lock the MHP sync RPCs so missing shells fail loudly instead of being silently recreated downstream.
