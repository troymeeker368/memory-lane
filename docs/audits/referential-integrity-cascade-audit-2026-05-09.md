...
Method: Reviewed current Supabase migrations, generated schema types, canonical services, tests, and current in-progress worktree diffs. This was not a live Supabase row scan, so findings below describe structural protections and structural gaps rather than live production row counts.

Change since last run:

- No new material referential-integrity regression was introduced by the current in-progress worktree.
- Enrollment packet public runtime hardening improved lifecycle truth handling: expired links are rejected earlier and follow-up-state persistence now fails loudly instead of silently degrading in [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime.ts), [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-context.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-context.ts), and [`/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts`](/D:/Memory Lane App/lib/services/enrollment-packets-public-runtime-cascade.ts).
- Enrollment-packet-generated lead activities now use the database-backed `lead_activities.idempotency_key` guard instead of relying only on pre-insert detection in [`/D:/Memory Lane App/lib/services/enrollment-packet-mapping-runtime.ts`](/D:/Memory Lane App/lib/services/enrollment-packet-mapping-runtime.ts).
- The core structural gaps from the 2026-05-08 audit remain open.
...
- The absence of a live row scan means this report cannot prove whether historical orphan or cross-member mismatch rows already exist in production.
...
1. Enrollment packet `completed` still does not mean downstream member conversion and follow-up are finished.
   ...
   - [`/D:/Memory Lane App/lib/services/enrollment-packet-readiness.ts`](/D:/Memory Lane App/lib/services/enrollment-packet-readiness.ts) still requires follow-up completion before `operationally_ready`.
   Why it matters:
   - "Enrollment packet completed without member creation" remains a structurally valid intermediate persisted state until downstream mapping and follow-up finish.
...
3. Public MHP->MCC sync still recreates an MHP shell instead of failing on missing upstream lifecycle state.
   ...
   - [`/D:/Memory Lane App/supabase/migrations/0206_fix_mhp_sync_member_id_ambiguity.sql`](/D:/Memory Lane App/supabase/migrations/0206_fix_mhp_sync_member_id_ambiguity.sql) begins by inserting into `public.member_health_profiles ... on conflict do nothing`.
   Why it matters:
   - Downstream sync can hide upstream provisioning drift instead of surfacing a missing canonical MHP shell for repair.

4. Signed-POF clinical sync also still recreates an MHP shell instead of requiring canonical shell provisioning first.
   Evidence:
   - [`/D:/Memory Lane App/supabase/migrations/0205_fix_signed_pof_sync_member_id_ambiguity.sql`](/D:/Memory Lane App/supabase/migrations/0205_fix_signed_pof_sync_member_id_ambiguity.sql) inserts into `public.member_health_profiles` before updating downstream clinical state.
   Why it matters:
...
- `member_health_profiles.member_id` remains one-to-one with the canonical member root.
...
1. `assessment_responses(assessment_id, member_id) -> intake_assessments(id, member_id)` is still missing.
   ...
   - [`/D:/Memory Lane App/types/supabase-types.d.ts`](/D:/Memory Lane App/types/supabase-types.d.ts) still exposes only `assessment_responses_member_id_fkey` plus the single-column assessment foreign key.
   Risk:
   - An intake response row can be relationally valid while pointing to an intake assessment owned by a different member.
...
2. `physician_orders(intake_assessment_id, member_id) -> intake_assessments(id, member_id)` is still missing.
   ...
   - [`/D:/Memory Lane App/types/supabase-types.d.ts`](/D:/Memory Lane App/types/supabase-types.d.ts) still exposes only `physician_orders_intake_assessment_id_fkey`.
   Risk:
...
5. Fence off or retire legacy mar_entries.
   Prompt:
...
6. Create a recurring live SQL lineage audit pack.
   Prompt:
...
The core lead-to-MAR chain still has important protections in place, and today’s worktree does not introduce a new relational regression in that chain. The most useful improvement since yesterday is that enrollment-packet public runtime truth got stricter: expired links now stop earlier, and failed follow-up-state persistence now fails loudly instead of quietly letting packet completion look healthier than it is.

The main production-readiness problem is still lifecycle truth rather than raw FK absence. A signed POF can still exist before downstream clinical sync finishes, and an enrollment packet can still be `completed` before mapping and follow-up are done. On top of that, the system still lets downstream sync RPCs recreate missing `member_health_profiles` shells on the fly, which hides upstream provisioning failures instead of surfacing them clearly.

The highest-value structural gap is still the missing composite lineage at the intake and physician-order boundaries. Right now, `assessment_responses`, `physician_orders`, `pof_requests`, and the POF post-sign queue each point at real parent rows, but the database still does not guarantee that those parent rows belong to the same member.

Next safe action:

1. Add the missing intake/member composite foreign keys first.
2. Re-lock both MHP shell-dependent sync RPCs so missing shells fail loudly again.
3. Add the missing composite physician-order lineage foreign keys.
4. Add durable source lineage on `member_medications`.
