...
left join public.member_command_centers mcc
  on mcc.member_id = m.id
where mcc.member_id is null
on conflict on constraint member_command_centers_member_id_key do nothing;
...
select
  'attendance-' || m.id::text,
  m.id,
  coalesce(m.enrollment_date, current_date),
  true,
  'BillNormally',
  coalesce(m.enrollment_date, current_date),
  0,
  ...
  coalesce(m.updated_at, now())
from public.members m
left join public.member_attendance_schedules mas
  on mas.member_id = m.id
where mas.member_id is null
on conflict on constraint member_attendance_schedules_member_id_key do nothing;
...
select
  ...
  null
from public.members m
left join public.member_health_profiles mhp
  on mhp.member_id = m.id
where mhp.member_id is null
on conflict on constraint member_health_profiles_member_id_key do nothing;
