...
begin
  ...
  select *
  into v_signature
  from public.intake_assessment_signatures
  ...
  if coalesce(v_assessment.signature_status, '') = 'signed' then
    ...
    set
      ...
      status = 'signed',
      signature_artifact_storage_path = coalesce(excluded.signature_artifact_storage_path, public.intake_assessment_signatures.signature_artifact_storage_path),
      signature_artifact_member_file_id = coalesce(excluded.signature_artifact_member_file_id, public.intake_assessment_signatures.signature_artifact_member_file_id),
