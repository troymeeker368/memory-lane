...
export async function buildCarePlanPdfDataUrl(carePlanId: string) {
  ...
  y -= 4;
  page.drawText("Sections", { x: 36, y, size: 12, font: fontBold, color: blue });
  y -= 16;
  ...
  detail.sections.forEach((section) => {
    if (y < 130) {
      ...
      page = next.page;
      y = next.y;
      page.drawText("Sections (continued)", { x: 36, y, size: 11, font: fontBold, color: blue });
      y -= 14;
    }

    page.drawText(section.sectionType, { x: 36, y, size: 10.8, font: fontBold, color: text });
    y -= 13;
  ...
  y = drawWrappedText({
    page,
    text: detail.carePlan.careTeamNotes || "-",
    x: 36,
    y,
    maxWidth: 540,
    lineHeight: 12,
