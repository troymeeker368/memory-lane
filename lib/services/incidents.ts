...
function asCategory(value: string): IncidentCategory {
...
function asDirectorDecision(value: string | null | undefined): IncidentDirectorDecision | null {
  const normalized = clean(value)?.toLowerCase();
  if (!normalized) return null;
  return (INCIDENT_DIRECTOR_DECISION_VALUES.find((item) => item === normalized) ?? null) as IncidentDirectorDecision | null;
}

function statusSortValue(status: IncidentStatus) {
  switch (status) {
    case "submitted":
      return 0;
    case "returned":
...
function assertDirectorReviewer(actor: ActorContext) {
...
function mapIncidentSummary(row: IncidentRow): IncidentSummaryRow {
  return {
    id: row.id,
...
function mapIncidentHistory(row: IncidentHistoryRow): IncidentHistoryEntry {
  return {
    id: row.id,
    action: row.action,
