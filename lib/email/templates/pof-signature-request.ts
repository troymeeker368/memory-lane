...
type BuildPofSignatureRequestTemplateInput = {
  providerName: string;
  nurseName: string;
  memberName: string;
  requestUrl: string;
...
// Provider-facing emails must use facility branding and must not expose internal platform names.
export function buildPofSignatureRequestTemplate(input: BuildPofSignatureRequestTemplateInput) {
  ...
  const html = `
    <div style="background:#f3f8fc;padding:24px;font-family:Arial,sans-serif;color:#0f172a;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #d9e4ef;border-radius:14px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:18px 24px 0;">
            <p style="margin:0 0 12px;font-size:15px;line-height:1.6;">This request was sent by ${nurseNameEscaped} from ${escapeHtml(facilityBranding.facilityName)}.</p>
