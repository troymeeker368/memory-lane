...
test("sales dashboard service reads stage counts and recent inquiries from the canonical dashboard RPC", () => {
  ...
  assert.equal(
    source.includes('import { getSalesDashboardSummarySupabase, normalizeSalesPipelineStageCounts } from "@/lib/services/sales-workflows";'),
    true
  );
  assert.equal(source.includes("async function getSalesDashboardSummarySupabase"), false);
