...
type PhotoWorkflowRow = Awaited<ReturnType<typeof getDocumentationWorkflows>>["photos"][number];
    ...
    <div className="space-y-4">
      ...
      <Card className="table-wrap">
        ...
        <table>
          ...
          <tbody>
            {workflows.photos.map((row: PhotoWorkflowRow) => (
              <tr key={row.id}><td>{formatDateTime(row.uploaded_at)}</td><td>{row.uploaded_by_name}</td><td>{row.file_name}</td><td>{row.file_type}</td><td><a href={row.photo_url ?? undefined} target="_blank" rel="noopener noreferrer">Open</a></td></tr>
            ))}
          </tbody>
        </table>
      </Card>
