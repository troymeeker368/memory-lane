    ...
    <div className="space-y-4">
      <Card>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <SalesMenuCard href={salesRoutes.pipelineIndex} label="Pipeline" subtitle={`${snapshot.openLeadCount} open leads across stage views`} icon={<GitBranch className="h-4 w-4" />} />
          ...
          <SalesMenuCard href={salesRoutes.activities} label="Recent Lead Activity" subtitle={`${snapshot.leadActivityCount} lead + ${snapshot.partnerActivityCount} partner activities`} icon={<Clock3 className="h-4 w-4" />} />
          {isAdmin ? (
            <SalesMenuCard href={salesRoutes.summary} label="Sales Summary" subtitle="Admin foundation dashboard for lead totals, stages, EIP, and inquiry activity" icon={<BarChart3 className="h-4 w-4" />} />
          ) : null}
        </div>
      </Card>
    </div>
