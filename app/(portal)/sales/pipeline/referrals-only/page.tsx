  ...
  return (
    <Card className="table-wrap">
      ...
      <table>
        ...
        <tbody>{rows.map((lead) => <tr key={lead.id}><td><Link className="font-semibold text-brand" href={`/sales/leads/${lead.id}`}>{lead.member_name}</Link></td><td>{lead.stage}</td><td>{lead.caregiver_name}</td><td>{lead.lead_source}</td><td>{lead.referral_name ?? "-"}</td><td>{lead.created_by_name}</td></tr>)}</tbody>
      </table>
    </Card>
  );
}
