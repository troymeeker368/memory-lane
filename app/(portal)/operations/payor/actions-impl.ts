...
async function handleSaveCenterBillingSetting(formData: FormData) {
  const profile = await requireBillingProfile();
...
async function handleSaveCenterClosure(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
  const now = toEasternISO();
...
async function handleSaveClosureRule(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
...
async function handleDeleteCenterClosure(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
...
async function handleEnsureCenterClosures(formData: FormData) {
  void formData;
  const profile = await requireBillingProfile();
  await ensureCenterClosuresForCurrentAndNextYear({
    generatedByUserId: profile.id,
...
async function handleSavePayor(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
  const now = toEasternISO();
...
async function handleSaveMemberBillingSetting(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
  const now = toEasternISO();
  const effectiveStartDate = asDateOnly(formData, "effectiveStartDate");
...
async function handleSaveBillingScheduleTemplate(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
  const now = toEasternISO();
  const effectiveStartDate = asDateOnly(formData, "effectiveStartDate");
...
async function handleSaveBillingAdjustment(formData: FormData) {
  const profile = await requireBillingProfile();
  const id = asString(formData, "id");
  const now = toEasternISO();
...
async function handleGenerateBillingBatch(formData: FormData) {
  try {
    const profile = await requireBillingProfile();
...
async function handleFinalizeBillingBatch(formData: FormData) {
...
async function handleReopenBillingBatch(formData: FormData) {
  const returnPath = asString(formData, "returnPath") || "/operations/payor/billing-batches";
  try {
...
async function handleFinalizeInvoice(formData: FormData) {
...
async function handleCreateBillingExport(formData: FormData) {
...
async function handleCreateCustomInvoice(formData: FormData) {
...
async function handleCreateEnrollmentInvoice(formData: FormData) {
  const returnPath = asString(formData, "returnPath") || "/operations/payor/custom-invoices";
  try {
    const profile = await requireBillingProfile();
...
const payorActionIntentSchema = [
  "saveCenterBillingSetting",
  "saveCenterClosure",
  "saveClosureRule",
  "deleteCenterClosure",
  "ensureCenterClosures",
  "savePayor",
  "saveMemberBillingSetting",
  "saveBillingScheduleTemplate",
...
type PayorActionIntent = (typeof payorActionIntentSchema)[number];

function resolvePayorActionIntent(formData: FormData): PayorActionIntent | null {
  const intent = asString(formData, "intent");
  return payorActionIntentSchema.includes(intent as PayorActionIntent) ? (intent as PayorActionIntent) : null;
}

export async function submitPayorActionImpl(formData: FormData) {
  const intent = resolvePayorActionIntent(formData);
  if (!intent) {
    redirectWithError(asString(formData, "returnPath") || "/operations/payor", "Unknown billing action.");
  }

  switch (intent) {
    case "saveCenterBillingSetting":
      return handleSaveCenterBillingSetting(formData);
    case "saveCenterClosure":
      return handleSaveCenterClosure(formData);
    case "saveClosureRule":
      return handleSaveClosureRule(formData);
    case "deleteCenterClosure":
      return handleDeleteCenterClosure(formData);
    case "ensureCenterClosures":
      return handleEnsureCenterClosures(formData);
    case "savePayor":
      return handleSavePayor(formData);
    case "saveMemberBillingSetting":
      return handleSaveMemberBillingSetting(formData);
    case "saveBillingScheduleTemplate":
      return handleSaveBillingScheduleTemplate(formData);
    case "saveBillingAdjustment":
      return handleSaveBillingAdjustment(formData);
    case "generateBillingBatch":
      return handleGenerateBillingBatch(formData);
    case "finalizeBillingBatch":
      return handleFinalizeBillingBatch(formData);
    case "reopenBillingBatch":
    ...
    case "createBillingExport":
      return handleCreateBillingExport(formData);
    case "createCustomInvoice":
      return handleCreateCustomInvoice(formData);
    case "createEnrollmentInvoice":
      return handleCreateEnrollmentInvoice(formData);
  }
}
